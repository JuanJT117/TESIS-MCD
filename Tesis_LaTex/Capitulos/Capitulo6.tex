...
%\textbf{Adquisición de datos y preprocesamiento: En primer lugar, se realiza la adquisición de datos in situ utilizando el método de Sondeo Eléctrico Vertical (SEV), el cual consiste en medir la resistividad del terreno a diferentes profundidades con un intervalo de muestreo predefinido. Este intervalo se determina de acuerdo con el objetivo de exploración, como la identificación de unidades geológicas, acuíferos, fallas, fracturas o estructuras antropogénicas. Para garantizar que los datos sean adecuados para el análisis, se lleva a cabo un preprocesamiento de los datos, que incluye la limpieza de valores atípicos, normalización de las lecturas y manejo de valores faltantes.}
	...
	\section{Preparación de datos}
			...
			Debido al bajo volumen de datos iniciales generados  en una campaña de exploración y su alto costo relativo, aunque significativo localmente para la interpretación de cada sitio, es insuficiente para establece una base de entrenamiento robusta, por lo que es imprescindible contar con las cantidad de datos suficiente para el entrenamiento.
	...
	\section{Evaluación, optimización y entrenamiento}
			...
			Parte del proceso de implementación de los modelos Machine Learning (RF, VSM, GBR), requiere de la evaluación mediante \textit{Cross-Validation} (GridSearchCV), la cual realiza 3 procesos iterativos, identifican el mejor ajuste de hiperparámetros que prioricen el desempeño, la reducción del sesgo y la maximización de las métricas de entrenamiento, esta proceso divide el conjunto de datos en k-pliegues (Particiones), en donde la fracción mayor corresponde al conjunto de entrenamiento y la mínima al de evaluación. 
			 
			En el proceso de evaluación del algoritmo GridSearchCV consideran un conjunto de entrenamiento de $k=3$ y $k=5$ pliegues. Esto permite analizar el equilibrio entre sesgo y varianza en la estimación del error predictivo de los datos de resistividad eléctrica \citep{hastie2009}.
...
%			Parte del proceso de entrenamiento de los modelos, RF, SVM y GBR, requiere ejecutar etapas de validación y optimización, en las cuales se evalúa el desempeño del entrenamiento para cada modelo bajo ciertos hiperparametros para conjuntos de entrenamiento establecidos por el algoritmo GridSearchCV, previo a la ejecución del proceso de entrenamiento el cual integra unicamente las variables con el mejor desempeño, este proceso permite ahorrar tiempo de procesamiento al establecer la configuración optima de hiperparametros.
			...
			El conjunto de hiperparametros corresponden a campos específicos para cada modelo, en la tabla \ref{tab:hiperparametros-modelos} se presentan el rango de valores evaluados en cada uno de los modelos ML.
	...
	\subsection{Análisis de generalización y rendimiento}
			...
			Esta baja sensibilidad a la partición de los datos en RF y GBR es evidencia de que logran capturar la complejidad global del fenómeno geoeléctrico sin incurrir en un sobreajuste significativo (\textit{overfitting}), comportándose estables y resistentes a la variabilidad de las distribuciones espaciales del conjunto de datos dada la convergencia de las métricas entre ambos escenarios vista en los modelos RF y GBR. Tal como demuestra \citet{kohavi1995} las particiones del orden de $k=5$ a $k=10$ son óptimas para estabilizar la varianza del modelo sin comprometer los tiempos de procesamiento, empleando convencionalmente la partición $k=5$ la que presenta el mejor equilibrio entre conjuntos de entrenamiento y evaluación.
			
			A partir del resultado de la evaluación del algoritmo GridSearchCV (Cross-Validation) empleando un valor de partición $k-pliegues=5$, se obtuvieron los hiperparámetros para cada modelo cuya generalización  
			...
			Una ves establecidos los hiperparametros idóneos se realiza un análisis global del desempeño de los modelos de Machine Learning evaluando la varianza del error predictivo aplicado a los 30 niveles de capas de regresión o columnas de pronostico.
			
			Por otro lado, el análisis de la varianza del error predictivo (evaluación por índice de columna de pronostico) expone la eficiencia del pronostico de los modelos. Al observar las métricas iterativas de MSE, Desviación Estándar y Curtosis, se hace evidente que el error predictivo del SVM se degrada sustancialmente (alcanzando valores de MSE superiores a $10,000$ en las columnas centrales). Esto se debe a que SVM, al ser un algoritmo de optimización de márgenes espaciales, tiende a perder precisión frente a las transiciones abruptas y capas fuertemente contrastantes típicas de los perfiles de resistividad, a menos que el hiperparámetro de su núcleo (\textit{kernel}) esté calibrado para cada estrato local de forma independiente \citep{Lary2016}.
			
			Contrariamente, tanto RF como GBR mantienen una traza de error estable a lo largo de todas las columnas de pronostico. La capacidad de los algoritmos basados en árboles para realizar particiones ortogonales y aislar subespacios locales les da una ventaja en el procesamiento con datos espaciales heterogéneos y variaciones estratigráficas pronunciadas \citep{Belgiu2016}. En particular, el enfoque de refuerzo secuencial de GBR \citep{Friedman2001} demuestra ser el más eficiente para corregir los residuos de predicción en zonas de alta incertidumbre, minimizando asimetrías y controlando la desviación estándar de los residuos de manera óptima a lo largo de todo el perfil. 
